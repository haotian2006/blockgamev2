...
if a then
	Block_Info = b
	for i,v in ipairs(game.ServerStorage.GenerationVersions:GetChildren()) do
...
function Generation.GetVersion(versiontouse)
	...
	local latest,touse= 0,"TerrainHandlerTemplate"--"pa_1.0-2.0"
		...
		for y,c in pairs(v) do
			for z,bdata in pairs(c) do
				...
				if (should and should[x..","..y..','..z]) or can(coord,Data,0,bdata,nearbychunks) or other[x..","..y..','..z] then
					...
					lc[x] = lc[x] or {}
					lc[x][y] = lc[x][y] or {}
					lc[x][y][z] =  {bdata[1],bdata[2],bdata[3],Chunk, not Block_Info[bdata[1]]["IsTransparent"]}
				end	
				size+= 1
...
function Generation.GetChunk(chunk,alrloaded)
	...
	genh =  allmoudlescripts[genh]
	if genh then
		data = genh.GetChunk(chunk,XZCoordInChunk(chunk))
	end
