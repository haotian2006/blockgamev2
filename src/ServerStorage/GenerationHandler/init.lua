...
function generation.Color(cx,cz,gtable):{}
	local function getColor(x,y,z)
		local combine = chunk.to1D(x,y,z)
		local self = gtable[combine]
		local above = gtable[chunk.to1D(x,y+1,z)]
		if not above and self then
			return 'T|s%C:Grass'
		elseif (above == 'T|s%C:Grass' or not gtable[chunk.to1D(x,y+3,z)]  ) and self  then
			return 'T|s%C:Dirt'
		elseif self then
			return'T|s%C:Stone'
		end
...
local function sphere(worms,Resolution,x,y,z)
		...
		for y1 = 1, Resolution*2 do
			for z1 = 1, Resolution*2 do
				...
				if Vector3.new(x1-Resolution,y1-Resolution,z1-Resolution).Magnitude <= Resolution-.1 and worldSpace.Y>=0 then
					...
					local localP = vector3int(lx,ly,lz)
					local comb = ccx..','..ccz
					worms[comb] = worms[comb] or {}
					--print(localP)
					worms[comb][tostring(chunk.to1D(localP.X,localP.Y,localP.Z))] = false --'air'
				end
			end
		end
	end
...
function generation.CreateWorms(cx,cz) -- cx and cy is the chunk it is being generated in
	...
	local ammountofcaves =  generation.proceduralNum(cx,cz,seed,20)
	local worms = {}
	...
	--for caves = 2,ammountofcaves+1 do
		...
		for ci = 1,ammount do
			...
			local maxlength = generation.proceduralNum(sx*.4,sz*.23,seed,300) --worm length
			local cwx,cwz = cx,cz
			local changed = 0
    ...
    for x = 0,st.ChunkSize.X-1 do
		for z = 0,st.ChunkSize.X-1 do
			...
			for y = max_height-1,0,-1 do
				...
				newtable[tostring(chunk.to1D(x,y,z))] = setblocktoston or d or nil
